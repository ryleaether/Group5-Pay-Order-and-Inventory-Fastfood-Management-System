...
class SidebarRenderer {
    ...
    public function __construct($admin_id, $fastfood_name = '', $admin_name = '', $username = '', $email = '') {
        ...
        // Auto-fetch from DB so every page always shows the correct name
        if (empty($admin_name) || empty($fastfood_name)) {
            $db   = new Database();
            $conn = $db->connect();
            $stmt = $conn->prepare("SELECT fullname, fastfood_name, username, email FROM admins WHERE admin_id = :id");
            $stmt->bindParam(":id", $admin_id);
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);

            if ($row) {
                $admin_name    = !empty($admin_name)    ? $admin_name    : $row['fullname'];
                $fastfood_name = !empty($fastfood_name) ? $fastfood_name : $row['fastfood_name'];
                $username      = !empty($username)      ? $username      : $row['username'];
    ...
    public function render($active_page = 'dashboard') {
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-profile" onclick="openAccountModal()" title="Click to edit profile">
                <div class="user-avatar"><?= $initial ?></div>
                <div class="user-info">
                    <div class="user-name"><?= $adminName ?></div>
                    <div class="user-role">Administrator</div>
                </div>
                <span class="profile-edit-btn" title="Edit profile">
                ...
                <ul>
                    <li>
                        <a href="#" onclick="openPinModal('cashier'); return false;" class="activity-link">
                            <span class="nav-icon">🖥️</span> Cashier Dashboard
                        </a>
                    </li>
                    <li>
                        <a href="#" onclick="openPinModal('kitchen'); return false;" class="activity-link">

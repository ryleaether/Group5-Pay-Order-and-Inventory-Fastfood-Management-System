...
<head>
    ...
    <style>
        ...
        .km-admin-modal h3 { font-size:18px; font-weight:700; color:#111; margin-bottom:4px; }
        .km-admin-modal p  { font-size:13px; color:#888; margin-bottom:0; }
    </style>
</head>
...
<div class="km-full-wrap">
    ...
    <!-- Top bar -->
    <div class="km-topbar">
        <div class="km-brand" id="kmBrand" onclick="kmBrandClick()" title="Click 5x to exit">
            🍳 <?= htmlspecialchars($_SESSION['fastfood_name'] ?? 'iPOS') ?> — Kitchen
        </div>
        <div class="km-topbar-right">
            <span class="km-live-dot"></span>
            <span id="lastRefresh">—</span>
            <button class="km-topbar-refresh" onclick="loadOrders()">🔄 Refresh</button>
        </div>
    </div>
...
<!-- Admin Exit Modal (5-click brand) -->
<div class="km-admin-overlay" id="kmAdminOverlay">
    <div class="km-admin-modal">
        <div style="font-size:30px; margin-bottom:8px;">🔒</div>
        <h3>Admin Access</h3>
        <p style="margin-bottom:16px;">Enter your PIN to exit Kitchen Mode</p>
        <div id="kmAdminDots" style="display:flex; justify-content:center; gap:10px; margin:14px 0;">
        ...
        <div class="pin-pad" style="max-width:220px; margin:0 auto;">
            <?php foreach([1,2,3,4,5,6,7,8,9,'',0,'⌫'] as $k): ?>
                <button type="button" class="pin-key"
                    onclick="<?= $k==='⌫' ? 'kmAdminBack()' : ($k==='' ? '' : "kmAdminPress($k)") ?>">
                    <?= $k ?>
                </button>
            <?php endforeach; ?>
        </div>
        <p id="kmAdminError" style="color:#dc2626; font-size:12px; margin-top:8px; display:none;">Incorrect PIN.</p>
        <button onclick="document.getElementById('kmAdminOverlay').style.display='none'; kmAdminPin=''; kmAdminUpdDots(0);"
                style="margin-top:12px; background:none; border:1px solid #ddd; padding:8px 20px; border-radius:8px; cursor:pointer; font-size:13px;">
            Cancel
        </button>
    </div>
</div>
...
// ===== 5-CLICK BRAND TO EXIT KITCHEN MODE =====
let kmBrandClicks = 0, kmBrandTimer = null;
function kmBrandClick() {
    kmBrandClicks++;
    clearTimeout(kmBrandTimer);
    if (kmBrandClicks >= 5) {
        kmBrandClicks = 0;
        kmAdminPin = ''; kmAdminUpdDots(0);
        document.getElementById('kmAdminError').style.display = 'none';
        document.getElementById('kmAdminOverlay').style.display = 'flex';
        return;
    }
    kmBrandTimer = setTimeout(() => { kmBrandClicks = 0; }, 2000);
}

let kmAdminPin = '';
function kmAdminPress(num) {
    if (kmAdminPin.length >= 4) return;
    kmAdminPin += String(num);
    kmAdminUpdDots(kmAdminPin.length);
    if (kmAdminPin.length === 4) setTimeout(kmAdminVerify, 100);
}
function kmAdminBack() { kmAdminPin = kmAdminPin.slice(0,-1); kmAdminUpdDots(kmAdminPin.length); }
function kmAdminUpdDots(n) {
    document.querySelectorAll('#kmAdminDots .pin-dot').forEach((d,i) => d.classList.toggle('filled', i<n));
}
function kmAdminVerify() {
    fetch('helpers/admindashboard_helpers.php?action=check_pin', {
        method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
        body:'pin='+encodeURIComponent(kmAdminPin)
    }).then(r=>r.json()).then(data => {
        if (data.success) {
            // Clear kitchen session lock and go to admin
            fetch('kitchen_pin_gate.php?action=clear_lock');
            window.location.href = 'admindashboard.php';
        } else {
            document.getElementById('kmAdminError').style.display = 'block';
            kmAdminPin = ''; kmAdminUpdDots(0);
            const d = document.getElementById('kmAdminDots');
